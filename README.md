...
9. 确认迁移，关闭旧 SSH 入口
10. 回退尚未确认的 SSH 迁移
11. 查看 Fail2ban 保护端口、封禁条件和封禁名单；端口与 SSH 不一致或没在运行时提示同步
12. 查看备份位置
...
从旧连接无法执行第 4 步。如果新连接失败，旧端口仍保留，可以选择 **10** 回退。回退会在 SSH 恢复旧端口后，删除迁移时为新端口添加的防火墙放行；迁移前就存在的规则不动。已经确认完成的迁移不支持菜单 10；要换回原端口，可再次使用菜单 8。
