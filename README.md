...
9. 查看 Fail2ban 保护端口、封禁条件和封禁名单，可直接解封 IP；端口与 SSH 不一致或没在运行时提示同步。识别模式用 aggressive：关闭密码登录后，攻击者拿真实用户名反复试密钥也会被封（默认的 normal 模式永远不封这类尝试）；只连端口不握手的探测几乎不计数，不会误封健康检查
